  ...
  document.getElementById('contentArea').innerHTML=`
    <div class="box" style="margin-top:0;">
      <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px;margin-bottom:14px;">
        ...
        <button class="success" style="width:auto;margin-top:0;padding:9px 18px;" onclick="abrirFormLancamento()">＋ Novo</button>
      </div>
...
function carregarLancamentos(){
  ...
  serverCall('Lancamento_listAll',[{estado:estado||undefined,mes:mes||undefined,ano:ano||undefined}],function(res){
    ...
    <div class="box" style="margin-top:16px;">
      ...
      <div class="section-grid">
        <div>
          ...
          <label>Igreja</label>
          <select id="fl_igreja"></select>
          <label>Departamento *</label>
          <select id="fl_depto"></select>
        </div>
        <div>
          <label>Grupo *</label>
          <select id="fl_grupo" onchange="actualizarRubricas()"></select>
          <label>Rubrica *</label>
          <select id="fl_rubrica"></select>
          <label>Conta *</label>
          <select id="fl_conta"></select>
      ...
      <div style="display:flex;gap:10px;flex-wrap:wrap;margin-top:14px;">
        ...
        <button class="info" style="width:auto;margin-top:0;padding:10px 20px;" onclick="guardarLancamento('RASCUNHO')">💾 Rascunho</button>
        <button class="success" style="width:auto;margin-top:0;padding:10px 20px;" onclick="guardarLancamento('SUBMETER')">📤 Submeter</button>
      </div>
      <div id="msgLanc" class="msg"></div>
    </div>`;

  getOpts(function(opts){
    if(!opts){ showMessage('❌ Erro ao carregar opções.', false, 'msgLanc'); return; }
    ...
    // Rubricas vazio até seleccionar grupo
    const selR=document.getElementById('fl_rubrica');
    if(selR) selR.innerHTML='<option value="">Seleccione o grupo primeiro</option>';
    // Contas
...
function guardarLancamento(modo){
  ...
  if(modo==='SUBMETER'){
    serverCall('Lancamento_create',[payload],function(res){
      ...
      if(!idLanc){ showMessage('✅ Criado e submetido!',true,'msgLanc'); setTimeout(()=>{ fecharFormLanc(); carregarLancamentos(); },1200); return; }
...
function submeterLancamento(id){
  ...
  serverCall('Lancamento_submit',[{id_lancamento:id}],function(){ carregarLancamentos(); },
...
function decidirAprovacao(idAprovacao,decisao){
  ...
  } else { if(!confirm('Confirmar aprovação?')) return; }
