...
function adminDiagnostico(){
  ...
  wrap.innerHTML = `
    <div class="box" style="margin-top:0;">
      ...
      <div style="background:#d1ecf1;border:1px solid #bee5eb;border-radius:8px;padding:12px;margin-bottom:16px;font-size:13px;color:#0c5460;">
        💡 <strong>Como encontrar o nome certo:</strong> Abra o seu Google Apps Script → veja as funções que começam com letras maiúsculas no ficheiro principal (ex: <code>doPost</code> chama funções como <code>Departamento_listar</code>). Pode também escrever qualquer nome suspeito no campo "Testar outra função" abaixo.
      </div>
      <div id="diagAuto"></div>
      <hr class="soft">
      <div style="margin-bottom:8px;">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;">
          <input type="text" id="diagFnInput" placeholder="Ex: Departamento_listar" style="flex:1;min-width:200px;padding:9px 12px;">
  ...
  const autoFns = [
    // Departamentos — variantes prováveis no padrão do backend
    { fn:'Departamento_listar',       titulo:'🏢 Departamentos (1)' },
    { fn:'Departamentos_getList',     titulo:'🏢 Departamentos (2)' },
    { fn:'Departamentos_obterTodos',  titulo:'🏢 Departamentos (3)' },
    { fn:'Departamento_obterTodos',   titulo:'🏢 Departamentos (4)' },
    { fn:'Departamentos_get',         titulo:'🏢 Departamentos (5)' },
    { fn:'Igreja_getDepartamentos',   titulo:'🏢 Departamentos (6)' },
    // Rubricas
    { fn:'Rubrica_listar',            titulo:'📂 Rubricas (1)' },
    { fn:'Rubrica_obterTodos',        titulo:'📂 Rubricas (2)' },
    { fn:'Rubricas_get',              titulo:'📂 Rubricas (3)' },
    { fn:'Rubrica_getList',           titulo:'📂 Rubricas (4)' },
    // Igrejas
    { fn:'Igreja_listar',             titulo:'⛪ Igrejas (1)' },
    { fn:'Igreja_obterTodos',         titulo:'⛪ Igrejas (2)' },
    { fn:'Igrejas_get',               titulo:'⛪ Igrejas (3)' },
    { fn:'Admin_getIgrejas',          titulo:'⛪ Igrejas (4)' },
  ];
...
function diagTestarExpandido(fn, titulo, container){
  ...
  serverCall(fn, [{}], function(res){
    ...
    // Tratar success:false como erro real
    if(res && res.success === false){
      el.innerHTML = `<div style="color:#721c24;padding:6px;background:#f8d7da;border-radius:6px;">❌ ${res.message||'Função não encontrada'}</div>`;
      return;
    }
    ...
    if(arr && arr.length){
      ...
      camposHtml = `<div style="padding:8px;background:#fff3cd;border-radius:8px;color:#856404;margin-bottom:8px;">
        ⚠️ Resposta não contém array. JSON completo:
      </div>
      <pre style="background:#f1f3f5;padding:8px;border-radius:6px;overflow-x:auto;font-size:11px;margin:0 0 8px;white-space:pre-wrap;word-break:break-all;">${jsonStr}</pre>`;
    }
...
function diagTestarUm(){
  ...
  wrap.innerHTML = '';
  diagTestarExpandido(fn, '🔧 Teste: ' + fn, wrap);
}
