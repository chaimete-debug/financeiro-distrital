...
function adminDiagnostico(){
  ...
  // Lista de candidatos a testar — nomes prováveis do backend
  const candidatos = [
    // Departamentos
    'Departamentos_listar','Departamento_listar','getDepartamentos',
    'listarDepartamentos','Departamentos_list','Departamentos_getAll',
    'getDepts','listDepartamentos','Departamentos_todos',
    // Rubricas / Categorias
    'Rubricas_listar','Rubrica_listar','getRubricas','listarRubricas',
    'Categorias_listar','Categoria_listar','getCategorias','listarCategorias',
    'Rubricas_list','Rubricas_getAll','getCategories','Rubricas_todos',
    // Igrejas
    'Admin_listarIgrejas','Igrejas_listar','getIgrejas','listarIgrejas',
    // Lançamentos
    'Lancamentos_listar','getLancamentos',
  ];

  wrap.innerHTML = `
    <div class="box" style="margin-top:0;">
      <h3>🔬 Diagnóstico do Backend</h3>
      <p class="muted">Clique em <strong>▶ Executar Todos</strong> para descobrir automaticamente quais funções existem no seu backend. Os resultados mostram o que cada função retorna.</p>
      <div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:16px;">
        <button class="success" style="width:auto;margin-top:0;padding:10px 20px;" onclick="diagCorrerTodos()">▶ Executar Todos</button>
        <button class="secondary" style="width:auto;margin-top:0;padding:10px 20px;" onclick="diagLimpar()">🗑️ Limpar</button>
      </div>
      <div style="margin-bottom:12px;">
        <label style="margin:0 0 6px;">🔧 Testar função específica:</label>
        <div style="display:flex;gap:8px;flex-wrap:wrap;">
          <input type="text" id="diagFnInput" placeholder="Ex: Departamentos_listar" style="flex:1;min-width:200px;padding:9px 12px;">
          <button class="info" style="width:auto;margin-top:0;padding:9px 18px;" onclick="diagTestarUm()">▶ Testar</button>
        </div>
      </div>
      <div id="diagResultados"></div>
    </div>`;
}

function diagLimpar(){
  const el = document.getElementById('diagResultados');
  if(el) el.innerHTML = '';
}
...
function diagTestarUm(){
  ...
  diagTestar(fn, document.getElementById('diagResultados'), true);
}

function diagCorrerTodos(){
  const wrap = document.getElementById('diagResultados');
  wrap.innerHTML = '<div class="muted">⏳ A testar ' + diagCandidatos().length + ' funções... aguarde.</div>';

  const grupos = {
    '🏢 Departamentos': diagCandidatos().filter(f=>f.toLowerCase().includes('depart')||f.toLowerCase().includes('dept')),
    '📂 Rubricas / Categorias': diagCandidatos().filter(f=>f.toLowerCase().includes('rubric')||f.toLowerCase().includes('categor')),
    '⛪ Igrejas': diagCandidatos().filter(f=>f.toLowerCase().includes('igreja')||f.toLowerCase().includes('igrej')),
    '📝 Outros': diagCandidatos().filter(f=>!f.toLowerCase().includes('depart')&&!f.toLowerCase().includes('dept')&&!f.toLowerCase().includes('rubric')&&!f.toLowerCase().includes('categor')&&!f.toLowerCase().includes('igreja')&&!f.toLowerCase().includes('igrej')),
  };

  wrap.innerHTML = '';
  Object.entries(grupos).forEach(([titulo, fns])=>{
    if(!fns.length) return;
    const secDiv = document.createElement('div');
    secDiv.style.marginBottom = '16px';
    secDiv.innerHTML = `<strong style="display:block;margin-bottom:8px;color:#2c3e50;">${titulo}</strong>`;
    wrap.appendChild(secDiv);
    fns.forEach(fn => diagTestar(fn, secDiv, false));
  });
}

function diagCandidatos(){
  return [
    'Departamentos_listar','Departamento_listar','getDepartamentos',
    'listarDepartamentos','Departamentos_list','Departamentos_getAll',
    'getDepts','listDepartamentos','Departamentos_todos',
    'Rubricas_listar','Rubrica_listar','getRubricas','listarRubricas',
    'Categorias_listar','Categoria_listar','getCategorias','listarCategorias',
    'Rubricas_list','Rubricas_getAll','Rubricas_todos',
    'Admin_listarIgrejas','Igrejas_listar','getIgrejas','listarIgrejas',
    'Lancamentos_listar','getLancamentos',
  ];
}

function diagTestar(fn, container, scroll){
  const rowId = 'diag_' + fn.replace(/[^a-zA-Z0-9]/g,'_');

  // remove entrada anterior se existir
  const prev = document.getElementById(rowId);
  if(prev) prev.remove();

  const row = document.createElement('div');
  row.id = rowId;
  row.style.cssText = 'background:#f8f9fa;border:1px solid #e9ecef;border-radius:10px;padding:12px;margin-bottom:8px;font-size:13px;';
  row.innerHTML = `<div style="display:flex;align-items:center;gap:8px;">
    <span id="${rowId}_icon">⏳</span>
    <code style="font-weight:700;color:#123b7a;">${fn}</code>
    <span id="${rowId}_status" style="color:#6c757d;">A testar...</span>
  </div>
  <div id="${rowId}_output" style="margin-top:6px;display:none;"></div>`;
  ...
  serverCall(fn, [{}],
    function(res){
      ...
      const output = document.getElementById(rowId+'_output');
      if(!icon) return;

      icon.textContent   = '✅';
      row.style.borderColor = '#28a745';
      status.style.color = '#155724';
      status.textContent = '— Função encontrada!';
      output.style.display = 'block';

      // Tentar extrair array de dados para mostrar estrutura
      let dados = res;
      if(res && typeof res === 'object'){
        const chaves = Object.keys(res);
        // procura a primeira chave que seja array
        for(const k of chaves){
          if(Array.isArray(res[k])){ dados = res[k]; break; }
        }
      }

      if(Array.isArray(dados) && dados.length){
        const amostra = dados[0];
        const campos  = Object.keys(amostra);
        output.innerHTML =
          `<div style="color:#155724;margin-bottom:4px;">📦 Retorna <strong>${dados.length}</strong> registo(s). Campos: <code>${campos.join(', ')}</code></div>` +
          `<details><summary style="cursor:pointer;color:#123b7a;">Ver 1º registo</summary><pre style="background:#e9f7ef;padding:8px;border-radius:6px;overflow-x:auto;font-size:11px;margin-top:4px;">${JSON.stringify(amostra, null, 2)}</pre></details>`;
      } else {
        output.innerHTML = `<details><summary style="cursor:pointer;color:#123b7a;">Ver resposta completa</summary><pre style="background:#e9f7ef;padding:8px;border-radius:6px;overflow-x:auto;font-size:11px;margin-top:4px;">${JSON.stringify(res, null, 2)}</pre></details>`;
      }
    },
    function(err){
      const icon   = document.getElementById(rowId+'_icon');
      ...
      status.textContent = '— Não encontrada ou erro: ' + (err?.message||'Falha');
    }
  );
}
